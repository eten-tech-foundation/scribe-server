...
import pino, { type BaseLogger, type Logger } from 'pino';
import env from '@/env';
...
if (env.NODE_ENV === 'production') {
  ...
  class AppInsightsLogger implements BaseLogger {
    ...
    info(...args: any[]): void {
      ...
      if (client)
        client.trackTrace({ message, severity: SeverityLevel.Information as any, properties });
    ...
    warn(...args: any[]): void {
      const { message, properties } = this.parseArgs(args);
      if (client)
        client.trackTrace({ message, severity: SeverityLevel.Warning as any, properties });
    ...
    debug(...args: any[]): void {
      const { message, properties } = this.parseArgs(args);
      if (client)
        client.trackTrace({ message, severity: SeverityLevel.Verbose as any, properties });
    ...
    fatal(...args: any[]): void {
      const { message, properties } = this.parseArgs(args);
      if (client)
        client.trackTrace({ message, severity: SeverityLevel.Critical as any, properties });
      console.error(message);
    }
