...
import { bible_texts, languages, project_units, projects, translated_verses } from './src/db/schema';
...
async function run() {
  ...
  try {
    ...
    allProjects.forEach(p => console.log(`Project ${p.id}: ${p.name} (Source: ${p.sourceLanguage}, Target: ${p.targetLanguage})`));
    ...
    allUnits.forEach(u => console.log(`Unit ${u.id} -> Project ${u.projectId} (Status: ${u.status})`));
    ...
    if (validTranslations.length > 0) {
      ...
      // Find its bible text
      const text = await db.select().from(bible_texts).where(eq(bible_texts.id, sample.bibleTextId));
